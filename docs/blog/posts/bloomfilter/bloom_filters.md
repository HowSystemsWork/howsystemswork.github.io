...
### Wait, what are Bloom Filters?

Bloom Filters are probabilistic data structures that quickly help determine if an element is present in a set or not.
In this post let's deep dive and understand how Bloom Filters work!

### Let's set a stage to discuss more...

Let's assume that you're given a word list containing 5 words, and that you're asked to find if another word given to
you
exists in the word list. How would you do it? A few ways to do this:

* search for each word in the word list from start to end (linear search)
* sort the words in the word list in alphabetical order and skip to that section of the list that's likely to contain
  the word (binary
  search).
* put the words in a hashmap and use `get()` on the hashmap
...
# or in simple one worder
def word_exists_ls_one_liner(input_word: str, word_list: List[str]) -> bool:
    # Note: The "in"  keyword performs a linear search through the list of words
...
The time complexity in this case is `O(logN)` where N is the number if strings in the word list.
...
def word_exists_hm(input_word: str, word_list: List[str]) -> bool:
    # Counter is an easy way to convert list to dict with count of each word
    # in the above case it will look like: Counter({'ape': 1, 'cat': 1, 'bear': 1, 'fish': 1, 'lion': 1})
...
### Everything fine right...? Why Bloom filters?
...
So, for `1 billion` strings a quick back of the napkin math would yield about `60 GB` memory (`60 bytes * 10 ^ 9`).
For `10 billion` strings, that numbers is `600 GB` and that's assuming that the strings are not longer than 10
characters.
What if there are unicode strings that are 20 characters long. Long story short, there should be a bound way to find out
if a string is in a set of `1 billion` or `10 billion` or even `100 billion` strings.
...
datastructure that could itself behave in a bound manner.

### How it works?

Let's learn about the working of Bloom Filters through the above example of the word list and the input word.

#### Insertion

To use a Bloom filter, we first need to insert all the words in our set. In our example w had 5 words. To do this we
do the following:
...
To check if a word exists or not in our word list, we then compute the three different hashes for our input word and
check if the index corresponding to the value of each has is actually set in the bit array.
So, for the word `cat`:
...
the word doesn't exist. Right? Let's assume that's the case. But what's the catch?
...
A keen eye would notice that the word `tiger` is not at all in our word list to begin with! In this case, the
bloom filter returns us a false sense of assurance that the word `tiger` is in the word list when it's the word `cat`
that's actually in the word list. So, the side effect of Bloom Filters is that they result in **false positives**.

But on the bright side, they never return false negatives. This is the catch and this is why we call Bloom Filters as a
probabilistic data structure as the result is an approximation.

> Deletion not supported!

One other characteristic of Bloom Filters is that we can only add elements to the them deletion is not supported. This
...
in real this array could be much longer. So, how do you actually decide how long your array should be?
...
### How do you calculate the probability of a false positives?
...
you decide which hash functions to choose? The hash function that you should use should be stable, provide an uniform
...
* Insertion: $O(k)$ to compute k number of hashes and check k bits
* Lookup: $O(k)$ to compute k number of hashes and set k bits
* Deletion: Not supported
...
* You don't worry about false positives i.e. you don't worry about getting accurate results.
...
It's already a long post, and we can definitely drill down deeper into some topics, but I'll try to do follow up
posts on some of the finer details of Bloom Filters which I found hard to cover in this post. I hope that the
information in this post is useful to get you all started on Bloom Filters :). I will also cite links to some
supplementary reading material that could very nicely complement this blog post down below.